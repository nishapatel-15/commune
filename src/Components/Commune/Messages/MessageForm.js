...
const MessageForm = props => {
	...
	const sendFile = () => {
		...
		task.on(
			"state_changed",
			(snap) => {
				var percentage = Math.round(snap.bytesTransferred / snap.totalBytes) * 100
	...
	const sendMessage = (message, serverID, categoryID, channelID) => {
		const path =
			"messages/" + (props.server ? props.server.serverId + "/" + props.server.channel.id : props.dm.id + "/messages")
		firebase.database().ref(path).push().set(message)
	}
